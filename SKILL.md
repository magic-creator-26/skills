...
1. **Ontology tags first.** If columns are tagged (`Phone`, `Name`, `NationalID`, `AccountID`,
   etc.), use those to identify what kind of entity the table tracks.
...
3. **Value samples third.** Look at actual cell values. Numeric sequences may be IDs; formatted
   strings may be phone numbers; short repeated strings may be categories or statuses.
4. **Cross-reference with other tables.** If a column in table A shares values with a column in
   table B (especially an ontology-tagged column), that is a join key and tells you something about
   both tables' purpose.
5. **Cross-reference with documents.** If a retrieved document mentions names or IDs that appear
   in a table, that table is likely relevant to the document's subject.
